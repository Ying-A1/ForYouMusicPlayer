...
void MainWidget::init_actions()
{
    ...
    QAction *action_play_to_favor=new QAction(QIcon(":/image/image/image/To-like.png"),u8"添加到我的收藏");
    ...
    QAction *action_local_to_favor=new QAction(QIcon(":/image/image/image/To-like.png"),u8"添加到我的收藏");
    ...
    QAction *action_musiclist_to_favor = new QAction(QIcon(":/image/image/image/To-like.png"), u8"添加到我的收藏");
...
void MainWidget::updateDuration(qint64 duration)
{
    ...
    if(!(static_cast<int>(duration) > 0)) {
        ...
        mySystemTray->setToolTip(u8"LightMusicPlayer · By Ying");
...
{
    QMessageBox::about(this,u8"关于","一款精致的本地音乐播放器\n"
                                   "作者：Ying\n\n"
                                   "【歌词文件说明】需要与对应的歌曲MP3在同目录且同名（.lrc文件）\n"
                                   "【快捷键说明】\n"
                                   "播放/暂停  -  空格\n"
                                   "上一曲/下一曲  -  Alt键+方向键←/→\n"
